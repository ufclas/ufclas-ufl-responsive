...
$calendar_xml_address = "http://www.google.com/calendar/feeds/".$googleaccount."/public/full?singleevents=true&max-results=50&futureevents=true&orderby=starttime&sortorder=ascending";
...
if (!is_wp_error( $feed )) { foreach ($feed->get_items() as $item) {
...
if($event_array_count==0) {echo "<p><em>No events currently scheduled. </em></p>"; }
...
foreach ($temp as $item) {
	...
	if($eventcount>1) {
		if (($eventcount>0 AND $items_shown<$eventcount)) {
	    ...
	    if (($event_date_converted-$todays_date)>=0 && ($event_date_converted-$todays_date)<=$eventdays) {
			if ($GroupByDate) {if ($gCalDate!=$old_date) { echo $temp_dateheader; $old_date=$gCalDate;}}
			echo $temp_event;
...
echo "</pre>"; }
	...
	echo '<p>See the <a href="' . esc_attr($url) . '">' . esc_html($title) . '</a> for the latest events.</p>';
    echo "<!--\n";
